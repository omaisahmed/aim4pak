...
class Cart extends Component {
    ...
    loadTranslations() {
        ...
        axios.get(`${baseUrl}admin/locale/cart`).then((res) => {
    ...
    loadProducts(search = "") {
        ...
        axios.get(`${baseUrl}admin/products${query}`).then((res) => {
    ...
    loadCart() {
        const baseUrl = document.querySelector('meta[name="base-url"]').getAttribute('content');
        axios.get(`${baseUrl}admin/cart`).then((res) => {
            const cart = res.data;
            this.setState({ cart });
        });
    }
    ...
    handleScanBarcode(event) {
        ...
        if (!!barcode) {
            axios
                .post(`${baseUrl}admin/cart`, { barcode })
                .then((res) => {
                    this.loadCart();
                    this.setState({ barcode: "" });
                })
    ...
    handleChangeQty(product_id, qty) {
        ...
        axios
            .post(`${baseUrl}admin/cart/change-qty`, { product_id, quantity: qty })
            .then((res) => {})
            .catch((err) => {
                Swal.fire("Error!", err.response.data.message, "error");
            });
    ...
    handleClickDelete(product_id) {
        const baseUrl = document.querySelector('meta[name="base-url"]').getAttribute('content');
        axios
            .post(`${baseUrl}admin/cart/delete`, { product_id, _method: "DELETE" })
            .then((res) => {
    ...
    handleEmptyCart() {
        const baseUrl = document.querySelector('meta[name="base-url"]').getAttribute('content');
        axios.post(`${baseUrl}admin/cart/empty`, { _method: "DELETE" }).then((res) => {
            this.setState({ cart: [] });
        });
    }
                    ...
                    <div className="order-product">
                        {products.map((p) => (
                            <div
                                ...
                                className="item"
                            >
                                <img src={`${baseUrl}${p.image_url}`} alt="Product | AIM4PAK" />
                                <h5 class="font-bold"
                                    style={
